...
async def get_mcp_tools():
    ...
    try:
        try:
            ...
            logger.error(
                f"{ERROR_ICON} Failed to import MultiServerMCPClient: {import_error}",
                exc_info=True,
            )
            _mcp_tools = None
            return []
        ...
        if not loaded_tools:
            ...
            _mcp_tools = None  # Do not cache empty list on failure, let it retry
            return []
        ...
        _mcp_tools = None  # Do not cache on failure, let it retry
        return []
