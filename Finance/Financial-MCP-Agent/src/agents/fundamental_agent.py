...
from src.schemas.agent_outputs import FundamentalAnalysisOutput
...
async def fundamental_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        try:
            ...
            if not mcp_tools:
                logger.error(
                    f"{ERROR_ICON} FundamentalAgent: No MCP tools available after filtering.")
                current_data["fundamental_analysis_error"] = "No MCP tools available after filtering."
                current_data["fundamental_analysis"] = f"基本面分析失败: 无法加载 MCP 工具集。这通常是由于 MCP 服务器 (`mcp_server.py`) 启动异常或语法错误导致的。请检查服务端日志。"
            ...
            # 构建精简的基本面分析请求
            agent_input = f"""请对{company_name}做简要基本面分析。

股票代码：{stock_code}（调用工具时请务必原样传入此代码，不要修改前缀）
当前日期：{current_date}

请依次调用以下工具（每类只调一次，不要重复调用）：
1. get_stock_basic_info(code="{stock_code}") - 获取公司基本信息
2. get_stock_industry(code="{stock_code}") - 获取所属行业
3. get_latest_trading_date - 获取最近交易日，确定最新报告期
4. get_profit_data(code="{stock_code}", year=最新年份, quarter=最新季度) - 盈利能力
5. get_growth_data(code="{stock_code}", year=最新年份, quarter=最新季度) - 成长能力
6. get_balance_data(code="{stock_code}", year=最新年份, quarter=最新季度) - 资产负债

拿到数据后，直接基于已有数据写出分析结论。如果某个工具返回空数据，跳过该维度即可，不要换季度重试。
...
不要使用 crawl_news 工具。不要编造数据。"""
            ...
            if not analysis_check.is_valid:
                ...
                logger.error(f"{ERROR_ICON} FundamentalAgent: {issue}")
                current_data["fundamental_analysis_error"] = issue
                final_output = build_agent_failure_message("基本面分析", issue, final_output)
            ...
            structured_output = await build_structured_output(
                llm=llm,
                schema_cls=FundamentalAnalysisOutput,
                agent_label="基本面分析",
                company_name=company_name,
                stock_code=stock_code,
                raw_analysis=final_output,
                issue=current_data.get("fundamental_analysis_error"),
            )

            # 8. 更新状态，保存分析结果和元数据
            current_data["fundamental_analysis"] = final_output
            current_data["fundamental_analysis_structured"] = structured_output.model_dump()
            current_metadata["fundamental_agent_executed"] = analysis_check.is_valid
            ...
            # 9. 添加消息记录，保持对话历史
            new_message = AIMessage(content="基本面分析已完成")
            ...
            }, total_execution_time, analysis_check.is_valid, current_data.get("fundamental_analysis_error"))
            ...
            current_data["fundamental_analysis_structured"] = FundamentalAnalysisOutput(
                company_name=sanitize_company_name(current_data.get("company_name")),
                stock_code=current_data.get("stock_code"),
                summary=None,
                missing_data=[str(e)],
                raw_analysis=current_data["fundamental_analysis"],
            ).model_dump()
        ...
        current_data["fundamental_analysis_structured"] = FundamentalAnalysisOutput(
            ...
            summary=None,
            missing_data=[str(e)],
            raw_analysis=current_data.get("fundamental_analysis", ""),
        ).model_dump()
        current_metadata["fundamental_agent_error"] = str(e)
...
async def test_fundamental_agent():
    ...
    print(result)
    print(result.data.get("fundamental_analysis", "No analysis found"))
