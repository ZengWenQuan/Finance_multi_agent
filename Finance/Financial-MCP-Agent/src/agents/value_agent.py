...
from src.schemas.agent_outputs import ValueAnalysisOutput
...
async def value_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        try:
            ...
            if not mcp_tools:
                logger.error(
                    f"{ERROR_ICON} ValueAgent: No MCP tools available after filtering.")
                current_data["value_analysis_error"] = "No MCP tools available after filtering."
                current_data["valuation_analysis"] = f"估值分析失败: 无法加载 MCP 工具集。这通常是由于 MCP 服务器 (`mcp_server.py`) 启动异常或语法错误导致的。请检查服务端日志。"
            ...
            # 构建精简的估值分析请求
            agent_input = f"""请对{company_name}做简要估值分析。

股票代码：{stock_code}（调用工具时请务必原样传入此代码，不要修改前缀）
当前日期：{current_date}

请依次调用以下工具（每类只调一次，不要重复调用）：
1. get_stock_basic_info(code="{stock_code}") - 获取公司基本信息（市值、股价等）
2. get_latest_trading_date - 获取最近交易日
3. get_dividend_data(code="{stock_code}", year=2025) - 获取股息数据

拿到数据后，基于市值、股价、股息收益率等可用指标，简要分析估值水平。
回答请控制在200字以内，不要重复同一句话。
...
不要使用 crawl_news 工具。不要编造数据。不要重复相同内容。"""
            ...
            if not analysis_check.is_valid:
                ...
                logger.error(f"{ERROR_ICON} ValueAgent: {issue}")
                current_data["value_analysis_error"] = issue
                final_output = build_agent_failure_message("估值分析", issue, final_output)
            ...
            structured_output = await build_structured_output(
                llm=llm,
                schema_cls=ValueAnalysisOutput,
                agent_label="估值分析",
                company_name=company_name,
                stock_code=stock_code,
                raw_analysis=final_output,
                issue=current_data.get("value_analysis_error"),
            )

            # 8. 更新状态，保存分析结果和元数据
            current_data["value_analysis"] = final_output
            current_data["value_analysis_structured"] = structured_output.model_dump()
            current_metadata["value_agent_executed"] = analysis_check.is_valid
            ...
            # 9. 添加消息记录，保持对话历史
            new_message = AIMessage(content="估值分析已完成")
            ...
            }, total_execution_time, analysis_check.is_valid, current_data.get("value_analysis_error"))
            ...
            current_data["value_analysis_structured"] = ValueAnalysisOutput(
                company_name=sanitize_company_name(current_data.get("company_name")),
                stock_code=current_data.get("stock_code"),
                summary=None,
                missing_data=[str(e)],
                raw_analysis=current_data["value_analysis"],
            ).model_dump()
        ...
        current_data["value_analysis_structured"] = ValueAnalysisOutput(
            ...
            summary=None,
            missing_data=[str(e)],
            raw_analysis=current_data.get("value_analysis", ""),
        ).model_dump()
        current_metadata["value_agent_error"] = str(e)
...
async def test_value_agent():
    ...
    print(result.data.get("value_analysis", "No analysis found"))
