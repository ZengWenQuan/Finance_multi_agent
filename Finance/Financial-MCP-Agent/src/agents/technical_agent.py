...
from src.schemas.agent_outputs import TechnicalAnalysisOutput
...
async def technical_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        try:
            ...
            if not mcp_tools:
                logger.error(f"{ERROR_ICON} TechnicalAgent: No MCP tools available after filtering.")
                current_data["technical_analysis_error"] = "No MCP tools available after filtering."
                current_data["technical_analysis"] = f"技术面分析失败: 无法加载 MCP 工具集。这通常是由于 MCP 服务器 (`mcp_server.py`) 启动异常或语法错误导致的。请检查服务端日志。"
            ...
            # 构建精简的技术分析请求
            agent_input = f"""请对{company_name}做简要技术面分析。

股票代码：{stock_code}（调用工具时请务必原样传入此代码，不要修改前缀）
当前日期：{current_date}

请依次调用以下工具（每类只调一次，不要重复调用）：
1. get_stock_basic_info(code="{stock_code}") - 获取公司基本信息和最新价格
2. get_latest_trading_date - 获取最近交易日
3. get_historical_k_data(code="{stock_code}", frequency="d", start_date=最近3个月前, end_date=最近交易日) - 获取日K线数据

拿到K线数据后，基于收盘价和成交量数据，在回复中分析：
- 近期价格趋势（涨跌方向、均线支撑）
- 成交量变化
- 基于数据给出短期走势判断

如果某个工具返回空数据，跳过即可，不要换参数重试。
...
不要使用 crawl_news 工具。不要编造数据。"""
            ...
            if not analysis_check.is_valid:
                ...
                logger.error(f"{ERROR_ICON} TechnicalAgent: {issue}")
                current_data["technical_analysis_error"] = issue
                final_output = build_agent_failure_message("技术分析", issue, final_output)
            ...
            structured_output = await build_structured_output(
                llm=llm,
                schema_cls=TechnicalAnalysisOutput,
                agent_label="技术分析",
                company_name=company_name,
                stock_code=stock_code,
                raw_analysis=final_output,
                issue=current_data.get("technical_analysis_error"),
            )

            # 8. 更新状态，保存分析结果和元数据
            current_data["technical_analysis"] = final_output
            current_data["technical_analysis_structured"] = structured_output.model_dump()
            current_metadata["technical_agent_executed"] = analysis_check.is_valid
            ...
            # 9. 添加消息记录，保持对话历史
            new_message = AIMessage(content="技术分析已完成")
            ...
            }, total_execution_time, analysis_check.is_valid, current_data.get("technical_analysis_error"))
            ...
            current_data["technical_analysis_structured"] = TechnicalAnalysisOutput(
                company_name=sanitize_company_name(current_data.get("company_name")),
                stock_code=current_data.get("stock_code"),
                summary=None,
                missing_data=[str(e)],
                raw_analysis=current_data["technical_analysis"],
            ).model_dump()
        ...
        current_data["technical_analysis_structured"] = TechnicalAnalysisOutput(
            ...
            summary=None,
            missing_data=[str(e)],
            raw_analysis=current_data.get("technical_analysis", ""),
        ).model_dump()
        current_metadata["technical_agent_error"] = str(e)
        execution_logger.log_agent_complete(agent_name, current_data, time.time() - agent_start_time, False, str(e))
        return AgentState(data=current_data, messages=current_messages, metadata=current_metadata)
...
async def test_technical_agent():
    ...
    print(result.data.get("technical_analysis", "No analysis found"))
