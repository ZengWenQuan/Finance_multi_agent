...
from src.utils.analysis_quality import sanitize_company_name, validate_analysis_output
from src.schemas.agent_outputs import SummaryAnalysisOutput
...
DEFAULT_SUMMARY_MAX_TOKENS = int(
    os.getenv("SUMMARY_MAX_TOKENS", os.getenv("OPENAI_COMPATIBLE_MAX_TOKENS", "6000"))
)


def _build_conservative_report(
    company_name: str,
    stock_code: str,
    ...
    lines = [
        f"# {company_name}({stock_code}) 综合分析报告",
        "",
        "## 执行摘要",
    ...
    for title, content in sections.items():
        ...
        lines.append(content.strip() if content.strip() else "未获取到可用结果。")
        lines.append("")

    lines.append("## 数据限制与风险")
    if issues:
        lines.extend(f"- {issue}" for issue in issues)
    else:
        lines.append("- 未发现额外限制。")
    lines.extend(["", f"分析基准时间：{current_time_info}"])
    return "\n".join(lines).strip()


def _is_usable_analysis(text: str, error: str | None) -> tuple[bool, str | None]:
    if error:
        return False, error
    result = validate_analysis_output(text)
    return result.is_valid, result.issue
...
def generate_report_with_finr1(model, tokenizer, prompt, max_new_tokens=3000):
    """使用FinR1模型生成报告"""
...
async def summary_agent(state: AgentState) -> AgentState:
    ...
    # 获取之前 Agent的分析结果
    fundamental_analysis = current_data.get("fundamental_analysis", "Not available")
    technical_analysis = current_data.get("technical_analysis", "Not available")
    value_analysis = current_data.get("value_analysis", "Not available")
    news_analysis = current_data.get("news_analysis", "Not available")
    structured_analysis_map = {
        "基本面分析结构化结果": current_data.get("fundamental_analysis_structured"),
        "技术分析结构化结果": current_data.get("technical_analysis_structured"),
        "估值分析结构化结果": current_data.get("value_analysis_structured"),
        "新闻分析结构化结果": current_data.get("news_analysis_structured"),
    }
    ...
    company_name = sanitize_company_name(current_data.get("company_name")) or "Unknown Company"
    ...
    try:
        ...
        analysis_inputs = {
            "基本面分析": (fundamental_analysis, current_data.get("fundamental_analysis_error")),
            "技术分析": (technical_analysis, current_data.get("technical_analysis_error")),
            "估值分析": (value_analysis, current_data.get("value_analysis_error")),
            "新闻分析": (news_analysis, current_data.get("news_analysis_error")),
        }
        valid_sections: dict[str, str] = {}
        section_issues: list[str] = list(errors)
        for title, (content, error) in analysis_inputs.items():
            ok, issue = _is_usable_analysis(content, error)
            if ok:
                valid_sections[title] = content
            else:
                section_issues.append(f"{title}不可用: {issue or '缺少可用内容'}")

        use_conservative_report = len(valid_sections) < 3
        validated_fundamental = valid_sections.get("基本面分析", "未获取到可用结果。")
        validated_technical = valid_sections.get("技术分析", "未获取到可用结果。")
        validated_value = valid_sections.get("估值分析", "未获取到可用结果。")
        validated_news = valid_sections.get("新闻分析", "未获取到可用结果。")
        ...
        **分析基准日期：{current_date}**
        ...
        请创建一份结构清晰、内容连贯的报告，整合所有可用分析的见解。
        如果某些分析数据不完整或缺失，必须直接说明“未获取到”，不能自行脑补。
        ...
        **严格遵循以下报告格式和结构：**
        ...
        ## 技术分析
        [仅总结输入中明确提供的价格趋势、技术指标、支撑位和阻力位、交易量等]
        
        ## 估值分析
        [仅总结输入中明确提供的估值指标、与行业平均水平比较、历史估值水平、股息收益率等]
        
        ## 新闻分析
        [仅总结输入中明确提供的市场情绪、重要新闻事件、媒体报道等；不要自行补充分析师评级变化]
        ...
        使用专业的金融语言，但保持可读性。报告只能引用输入中明确出现的数据、比率、日期、价格和结论。
        如果输入里没有具体数值，不得生成具体数值，不得生成目标价、PE、PS、分析师评级或未来季度数据。
        ...
        **重要提醒：**
        ...
        # 准备汇总提示词
        user_prompt = f"""
        Please create a comprehensive analysis report for {company_name} ({stock_code}) based only on the validated analyses below.
        
        Original user query: {user_query}
        
        VALIDATED FUNDAMENTAL ANALYSIS:
        {validated_fundamental}
        ...
        and formatting. If a section lacks validated input, write "未获取到可用结果" instead of guessing.
        ...
        structured_sections = []
        for title, payload in structured_analysis_map.items():
            if payload:
        ...
        if use_conservative_report:
            final_report = _build_conservative_report(
                company_name=company_name,
                stock_code=stock_code,
                current_time_info=current_time_info,
                sections={
                    "基本面分析": valid_sections.get("基本面分析", "未获取到可用结果。"),
                    "技术分析": valid_sections.get("技术分析", "未获取到可用结果。"),
                    "估值分析": valid_sections.get("估值分析", "未获取到可用结果。"),
                    "新闻分析": valid_sections.get("新闻分析", "未获取到可用结果。"),
                },
                issues=section_issues,
            )
            llm_execution_time = 0.0
            model_config = {"mode": "conservative_fallback"}
            ...
            # 使用FinR1模型生成最终报告
            final_report = generate_report_with_finr1(
                model,
                tokenizer,
                full_prompt,
                max_new_tokens=DEFAULT_SUMMARY_MAX_TOKENS
            )
            ...
            if not all([api_key, base_url, model_name]):
                ...
                return AgentState(data=current_data, messages=messages, metadata=current_metadata)
        ...
        structured_summary = await build_structured_output(
            llm=llm if not use_conservative_report and model_choice != "local" else None,
            schema_cls=SummaryAnalysisOutput,
            agent_label="综合报告",
            company_name=company_name,
            stock_code=stock_code,
            raw_analysis=final_report,
            issue="; ".join(section_issues) if section_issues else None,
        )

        supported_source_text = "\n".join(valid_sections.values())
        unsupported_terms = ["目标价", "分析师评级", "市盈率", "市销率", "PS", "PE", "Q3", "季度"]
        leaked_terms = [
            term for term in unsupported_terms
            if term in final_report and term not in supported_source_text
        ]
        if leaked_terms:
            section_issues.append(f"汇总报告包含未在上游结果中出现的内容: {', '.join(leaked_terms)}")
            final_report = _build_conservative_report(
                company_name=company_name,
                stock_code=stock_code,
                current_time_info=current_time_info,
                sections={
                    "基本面分析": validated_fundamental,
                    "技术分析": validated_technical,
                    "估值分析": validated_value,
                    "新闻分析": validated_news,
                },
                issues=section_issues,
            )
        ...
        current_data["summary_analysis_structured"] = structured_summary.model_dump()
        current_data["report_path"] = report_path
        current_metadata["summary_agent_executed"] = True
        current_metadata["summary_agent_timestamp"] = str(time.time())
        current_metadata["summary_agent_execution_time"] = f"{llm_execution_time:.2f} seconds"
        updated_messages = list(messages) + [AIMessage(content="汇总报告已生成")]
        ...
        return AgentState(data=current_data, messages=updated_messages, metadata=current_metadata)
        ...
        **Error encountered during report generation**: {e}
        ...
        current_data["summary_analysis_structured"] = SummaryAnalysisOutput(
            company_name=company_name,
            stock_code=stock_code,
            summary=None,
            data_limitations=[str(e)],
            raw_analysis=error_report,
        ).model_dump()
        ...
        current_data["report_path"] = report_path
        current_metadata["summary_agent_error"] = str(e)
...
async def test_summary_agent():
    ...
    print(result.data.get("final_report", "No report generated"))
    print(
        f"Report saved to: {result.data.get('report_path', 'Not saved')}")
