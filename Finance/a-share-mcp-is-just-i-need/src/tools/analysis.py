...
def _get_latest_available_financial_period(now: datetime) -> tuple[str, int]:
    """
    Return the latest quarter that is normally available from public filings.

    Chinese quarterly report deadlines are typically:
    - Q1: Apr 30
    - Q2: Aug 31
    - Q3: Oct 31
    - Q4 annual: Apr 30 of next year
    """
    year = now.year
...
def register_analysis_tools(app: FastMCP, active_data_source: FinancialDataSource):
    ...
    def get_stock_analysis(code: str, analysis_type: str = "fundamental") -> str:
        """
        [1. 核心定义]
        一键生成特定股票的【综合分析报告】（涵盖基本面和技术面聚合）。

        [2. 适用场景与触发词]
        - 当用户需要全方位的“诊断”、“分析报告”、“综合评判”或“调研简报”。
        
        [3. 排他约束 (防混淆)]
        - ⚠️ 若用户要求“分析”、“诊断”、“调研”、“评估”或“出报告”，请务必使用此工具，不要使用简单的查询工具。
        - 如果用户只问了具体的某个财务指标数值（例如“ROE是多少”），请不要使用此工具，应单独调取对应财务工具。
        - 此工具消耗较大，请只在用户明确想要“分析报告”时使用。
        """
        logger.info(
        ...
        try:
            ...
            if analysis_type in ["fundamental", "comprehensive"]:
                # 获取最近一个季度财务数据
                recent_year, recent_quarter = _get_latest_available_financial_period(datetime.now())
