...
def register_date_utils_tools(app: FastMCP, active_data_source: FinancialDataSource):
    ...
    def get_latest_trading_date() -> str:
        ...
        [2. 适用场景与触发词]
        当你在调用其他行情数据或宏观数据工具需要知道“昨天是不是交易日”、“当前开盘截止日是哪天”时，请调用。
        
        [3. 排他约束]
        ⚠️ 注意：这是一个纯粹的辅助时间工具。它不返回任何股票的股价或行情数据！如果你想查 K 线或收盘价，请使用 get_historical_k_data！
        """
    ...
    def get_market_analysis_timeframe(period: str = "recent") -> str:
        """
        [1. 核心定义]
        获取适合宏观或市场分析的【建议时间区间段】字符串。

        [2. 适用场景]
        当用户问题中对时间表述比较模糊（例如“最近走势怎样”、“今年大概什么情况”），你需要计算出一个合规的 start_date 和 end_date 给别的工具时调用。
        """
        logger.info(
