...
def _fallback_markdown_table(df: pd.DataFrame) -> str:
    """Render a simple Markdown table when pandas.to_markdown dependencies are missing."""
...
def format_df_to_markdown(df: pd.DataFrame, max_rows: int = None) -> str:
    ...
    try:
        ...
        logger.warning(
            f"tabulate is unavailable, falling back to internal markdown renderer: {e}"
        )
        markdown_table = _fallback_markdown_table(df_display)
